...
class Coord:
    ...
    def Square(self):
        try:
            ...
            self.square=Coord(
                [self.numer[i]**2 for i in range(0,4)],self.denom**2,self.field)
            return self.square
    ...
    def Sum_chit_thetaipt_thetat(self):
        ...
        try:
            ...
            self.sctt_ichi={}
            #[(i,chi)]
            self.sctt_ichi[(1,0)]=2*prod_00_10+2*prod_01_11
            self.sctt_ichi[(2,0)]=2*prod_00_01+2*prod_10_11
            self.sctt_ichi[(3,0)]=2*prod_00_11+2*prod_10_01
            self.sctt_ichi[(1,2)]=2*prod_00_10-2*prod_01_11
            self.sctt_ichi[(2,1)]=2*prod_00_01-2*prod_10_11
    ...
    def Mult_frac(self,frac:list):
        ...
        """
        return Coord([self.numer[i]*frac[0] for i in range(0,4)],
                     self.denom*frac[1],
                     self.field)
    ...
    def Common_denom(self,tc_2):
        ...
        new_tc_1=Coord([self.numer[i]*tc_2.denom for i in range(0,4)],
                       d,self.field)
        new_tc_2=Coord([tc_2.numer[i]*self.denom for i in range(0,4)],
                       d,self.field)
        return new_tc_1,new_tc_2

    def Multiply(self,tc_2):
        """ multiply for each component."""
        prod_tc=Coord([self.numer[i]*tc_2.numer[i] for i in range(0,4)],
                      self.denom*tc_2.denom,self.field)
        return prod_tc
    ...
    def Divide(self,tc_2): #self[i]/tc_2[i]
        ...
        tc_1div2=Coord([num_frac[i][0]*tc_2.denom for i in range(0,4)],
                       den*self.denom,self.field)
        return tc_1div2
...
class NullCoord(Coord):
    ...
    def Kappa_ij(self,tc_x:Coord,tc_y:Coord):
        ...
        #z_i_chi.
        z_10_00=[sctt_ichi_x[(1,0)]*sctt_ichi_y[(1,0)],sctt_ichi_0[(1,0)]]
        z_10_01=[sctt_ichi_x[(1,2)]*sctt_ichi_y[(1,2)],sctt_ichi_0[(1,2)]]
        z_01_00=[sctt_ichi_x[(2,0)]*sctt_ichi_y[(2,0)],sctt_ichi_0[(2,0)]]
        z_01_10=[sctt_ichi_x[(2,1)]*sctt_ichi_y[(2,1)],sctt_ichi_0[(2,1)]]
        ...
        kappa_ij[(0,1)]=[z_10_00[0]+z_10_01[0],4*den_10]
        kappa_ij[(0,2)]=[z_01_00[0]+z_01_10[0],4*den_01]
        kappa_ij[(0,3)]=[z_11_00[0]+z_11_11[0],4*den_11]
        kappa_ij[(1,2)]=[z_11_00[0]-z_11_11[0],4*den_11]
        kappa_ij[(1,3)]=[z_01_00[0]-z_01_10[0],4*den_01]
        kappa_ij[(2,3)]=[z_10_00[0]-z_10_01[0],4*den_10]
        return kappa_ij
    ...
    def Normal_Add(self,tc_x:Coord,tc_y:Coord,number:int):
        ...
        alpha=1
        kappa_a0sq=[
            kappa_ij[(0,alpha)][0]**2,
            kappa_ij[(0,alpha)][1]**2]          
        kappa_aa00=[
            kappa_ij[(alpha,alpha)][0]*kappa_ij[(0,0)][0],
            kappa_ij[(alpha,alpha)][1]*kappa_ij[(0,0)][1]]
    ...
    def Diff_Add_except(self,tc_x:Coord,tc_y:Coord,tc_xmy:Coord):
        ...
        frac_xpy=[[0,0] for counter in range(0,4)]
    ...
    def Is_on_Kummer_eq(self,lv2_tc:Coord):
        #assert (not lv2_tnp.Is_elliptic_prod_without_theta())
        [a,d,b,c]=self.numer
        [x,t,y,z]=lv2_tc.numer
        Asq=(a**2+b**2+c**2+d**2)/4
        Bsq=(a**2+b**2-c**2-d**2)/4
        Csq=(a**2-b**2+c**2-d**2)/4
        Dsq=(a**2-b**2-c**2+d**2)/4
        d1=a**2*d**2-b**2*c**2
        d2=a**2*c**2-b**2*d**2
        d3=a**2*b**2-c**2*d**2
        E=(256*a*b*c*d*Asq*Bsq*Csq*Dsq)/(d1*d2*d3)
        F=(a**4-b**4-c**4+d**4)/d1
        G=(a**4-b**4+c**4-d**4)/d2
        H=(a**4+b**4-c**4-d**4)/d3
        Eq=(x**4+y**4+z**4+t**4)+2*E*x*y*z*t-F*(x**2*t**2+y**2*z**2)-G*(x**2*z**2+y**2*t**2)-H*(x**2*y**2+z**2*t**2)
        return Eq==0
        
        


def To_null(lv2_tnp:Coord):
    """ 
    For theta-null point of type()==Coord,
    we change the type to NullCoord.
    """
    return NullCoord(lv2_tnp.numer,lv2_tnp.denom,lv2_tnp.field)
