...
def Half_LinCom_dim1(tc_0:Dim1_theta_null,ext_basis:list,l:int):
    ...
    for k2 in range(2,ld+2):
        ...
        lincom[(0,k2)]=tc_0.Diff_add_dim1(
            lincom[(0,k2-1)],tc_e2,lincom[(0,k2-2)])
    for k2 in range(2,l-1):
        assert(not (1,k2) in lincom)
        lincom[(1,k2)]=tc_0.Diff_add_dim1(
            lincom[(1,k2-1)],tc_e2,lincom[(1,k2-2)])
    for k1 in range(2,ld+2):
        assert(not (k1,0) in lincom)
        lincom[(k1,0)]=tc_0.Diff_add_dim1(
            lincom[(k1-1,0)],tc_e1,lincom[(k1-2,0)])
    for k1 in range(2,l):
        assert(not (k1,1) in lincom)
        lincom[(k1,1)]=tc_0.Diff_add_dim1(
            lincom[(k1-1,1)],tc_e1,lincom[(k1-2,1)])
    for k1 in range(2,l-1):
        assert(not (k1,2) in lincom)
        lincom[(k1,2)]=tc_0.Diff_add_dim1(
            lincom[(k1-1,2)],tc_e1,lincom[(k1-2,2)])
    for k1 in range(2,ld+1):
        for k2 in range(3,l-k1):
            assert(not (k1,k2) in lincom)
            lincom[(k1,k2)]=tc_0.Diff_add_dim1(
...
def XpLinCom_dim1(tc_0:Dim1_theta_null,ext_basis:list,ext_x:list,l:int):
    """ 
    ...
    tc_xpe12=tc_0.Three_way_dim1(
        tc_x,tc_e1,tc_e2,tc_xpe1,tc_e12,tc_xpe2)
    ...
    xplincom={(0,0):tc_x,
              ...
              (1,1):tc_xpe12}
    for k2 in range(2,l+1):
        xplincom[(0,k2)]=tc_0.Diff_add_dim1(
            xplincom[(0,k2-1)],tc_e2,xplincom[(0,k2-2)])
    for k2 in range(2,l):
        xplincom[(1,k2)]=tc_0.Diff_add_dim1(
            xplincom[(1,k2-1)],tc_e2,xplincom[(1,k2-2)])
    for k1 in range(2,l+1):
        xplincom[(k1,0)]=tc_0.Diff_add_dim1(
            xplincom[(k1-1,0)],tc_e1,xplincom[(k1-2,0)])
    for k2 in range(1,l):
        for k1 in range(2,l):
