...
def Dict_common_denom(lincom:dict):
    ...
    for k in lincom.keys():
        lincom[k].numer=[
            dict_num[k]*lincom[k].numer[j] for j in range(0,4)]
        lincom[k].denom=den
    assert(den!=0)
    return lincom,den
...
def Dict_common_denom_len3(lincom:dict):
    """ 
    the type of input "lincom" is "dict".
    for all keys of the dict, lincom[key]=[n0,n1,d] of length 3. 
    here, "n" means "numerator", "d" means "denominator".
    We output new_lincom:dict s.t. new_lincom[key][i]/new_lincom[key][2]==lincom[key][i]/lincom[key][2] for i=0,1.
    in addition, for all key, new_lincom[key][2] is the same.
    """
    assert (all(len(lincom[key])==3 for key in lincom))
    list_lincom=sorted(lincom.items())
    k=len(list_lincom)
    list_denoms=[list_lincom[i][1][2] for i in range(0,k)]
    list_mult_num,den=Common_Denom(list_denoms)
    dict_mult_num={
        list_lincom[i][0]:list_mult_num[i] for i in range(0,k)}
    for key in lincom.keys():
        lincom[key]=[
            dict_mult_num[key]*lincom[key][i] for i in range(0,2)]
...
def Dict_common_denom_len5(lincom:dict):
    ...
    for key in lincom.keys():
        lincom[key]=[
            dict_mult_num[key]*lincom[key][i] for i in range(0,4)]
        lincom[key].append(den)
    return lincom,den
